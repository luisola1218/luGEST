...
from lugest_qt.services.quote_editor_composition import profile_editor_ports
from lugest_qt.services.quote_editor_composition import quote_editor_ports
...
class QuotesPage(QWidget):
    ...
    def _structure_line(self, descricao: str, qtd: float, unid: str, preco_unit: float, operacao: str) -> dict | None:
        return service_line(descricao, qtd, unid, preco_unit, operacao, line_type=self.backend.desktop_main.ORC_LINE_TYPE_SERVICE)

    def _structure_product_line(self, product: dict | None, qtd: float, *, descricao_extra: str = "") -> dict | None:
        return product_line(product, qtd, descricao_extra=descricao_extra, line_type=self.backend.desktop_main.ORC_LINE_TYPE_PRODUCT)

    def _structure_quote_dialog(self) -> dict | None:
        return edit_structure(self, quote_editor_ports(self.backend), workcenter=self._quote_pick_workcenter("Serralharia", "Montagem"))
    ...
    def _material_price_manager_dialog(self, formato_filter: str = "", preferred_id: str = "", parent: QWidget | None = None) -> dict | None:
        return edit_material_prices(self, quote_editor_ports(self.backend), formato_filter, preferred_id, parent)
    ...
    def _sync_stock_price_from_context(self, material_id: str, price_kg: float, current_price_kg: float, parent: QWidget | None = None) -> dict[str, Any] | None:
        return sync_stock_price(self, quote_editor_ports(self.backend), material_id, price_kg, current_price_kg, parent)

    def _material_assembly_item_dialog(self, initial: dict | None = None, parent: QWidget | None = None) -> dict | None:
        return edit_material(self, quote_editor_ports(self.backend), initial, parent, presets=self.presets)

    def _labor_assembly_item_dialog(self, initial: dict | None = None, parent: QWidget | None = None) -> dict | None:
        return edit_labor(self, quote_editor_ports(self.backend), initial, parent, presets=self.presets)

    def _consumable_assembly_item_dialog(self, initial: dict | None = None, parent: QWidget | None = None) -> dict | None:
        return edit_consumable(self, quote_editor_ports(self.backend), initial, parent)

    def _product_assembly_item_dialog(self, initial: dict | None = None, parent: QWidget | None = None) -> dict | None:
        return edit_product(self, quote_editor_ports(self.backend), initial, parent)
    ...
    def _line_dialog(self, initial: dict | None = None, *, template_mode: bool = False) -> dict | None:
        return edit_line(self, quote_editor_ports(self.backend), initial, template_mode=template_mode, presets=self.presets, client_code=self._client_code_from_text(self.client_combo.currentText()), current_number=self.current_number, line_rows=self.line_rows)
    ...
    def _open_profile_step_igs_quote_builder(
        ...
        *,
        ...
        lines = edit_profile(self, profile_editor_ports(self.backend), parent=parent)
        if return_lines:
            return lines or []
        if lines is None:
            return None
