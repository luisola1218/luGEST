...
from lugest_qt.services.purchasing_composition import note_lifecycle, note_commands, purchase_lines
...
class PurchasingBridgeMixin:
    ...
    def _infer_purchase_material_line(self, line: dict[str, Any]) -> dict[str, Any]:
        return purchase_lines(self).infer_material(line)
            ...
            rows.append(
                {
                    ...
                    "preco_venda": round(self._parse_float(raw.get("preco_venda", raw.get("pvp1", raw.get("preco_unid", 0))), 0), 4),
                    "p_compra": round(self._parse_float(raw.get("p_compra", 0), 0), 4),
                    "pvp1": round(self._parse_float(raw.get("pvp1", 0), 0), 4),
                    "pvp2": round(self._parse_float(raw.get("pvp2", 0), 0), 4),
                    "categoria": categoria,
                    "tipo": tipo,
    ...
    def _ne_normalize_line(self, payload: dict[str, Any]) -> dict[str, Any]:
        return purchase_lines(self).normalize(payload)

    def ne_save(self, payload: dict[str, Any]) -> dict[str, Any]:
        return note_commands(self).save(payload)

    def ne_create_draft(self) -> dict[str, Any]:
        return note_lifecycle(self).create_draft()

    def ne_remove(self, numero: str) -> None:
        return note_lifecycle(self).remove(numero)

    def ne_approve(self, numero: str) -> dict[str, Any]:
        return note_lifecycle(self).approve(numero)

    def ne_mark_sent(self, numero: str) -> dict[str, Any]:
        return note_lifecycle(self).mark_sent(numero)

    def ne_generate_supplier_orders(self, numero: str) -> list[dict[str, Any]]:
        return note_lifecycle(self).generate_orders(numero)
