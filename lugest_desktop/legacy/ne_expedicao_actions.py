...
def _pdf_brand_palette():
    ...
    try:
        ...
        pass
    primary_soft = _pdf_mix_hex(primary_hex, "#FFFFFF", 0.84)
    primary_soft_2 = _pdf_mix_hex(primary_hex, "#FFFFFF", 0.94)
    primary_mid = _pdf_mix_hex(primary_hex, "#FFFFFF", 0.58)
    line_hex = "#CAD3DA"
    return {
        ...
        "primary_mid": colors.HexColor(primary_mid),
...
def _pdf_draw_soft_header_panel(c, page_h, x, top_y, width, height, radius=4):
    from reportlab.lib import colors

    try:
        page_w = float((getattr(c, "_pagesize", (0, 0)) or (0, 0))[0] or 0)
        if page_w > 0:
            c.setFillColor(colors.white)
            c.rect(0, 0, page_w, page_h, stroke=0, fill=1)
            c.setFillColor(_pdf_brand_palette()["primary"])
...
def _render_expedicao_pdf_modern(self, path, ex, include_all_vias=True):
    ...
    def draw_first_header(page_no, total_pages, via_nome):
        ...
        _pdf_draw_soft_header_panel(c, h, banner_x, 18, banner_w, 80, radius=4)
        logo_x = m
...
def _render_ne_pdf_modern(self, path, ne):
    ...
    def draw_header(page_no, total_pages, first_page):
        if first_page:
            ...
            banner_x = m + logo_w + logo_gap
            banner_w = content_w - logo_w - logo_gap
            _pdf_draw_soft_header_panel(c, h, banner_x, 20, banner_w, 82, radius=4)
            logo_x = m
