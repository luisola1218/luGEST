...
class QuotePage(QWidget):
    ...
    def _calculated_assembly_builder_dialog(self, initial: dict | None = None) -> dict | None:
        ports = CalculatedAssemblyPorts(
            quote_number=self.current_number,
            workcenter=self.view.workcenter_combo.currentText().strip(),
            save_pair=self.services.save_assembly_pair,
            conjunto_next_param_codigo=self.services.conjunto_next_param_codigo,
            laser_batch_dialog=self.services.laser_batch_dialog,
            norm_text=self.services.norm_text,
            orc_line_is_piece=self.services.orc_line_is_piece,
            _assembly_item_kind_from_line=self._assembly_item_kind_from_line,
            _consumable_assembly_item_dialog=self._consumable_assembly_item_dialog,
            _edit_laser_batch_lines=self._edit_laser_batch_lines,
            _labor_assembly_item_dialog=self._labor_assembly_item_dialog,
            _material_assembly_item_dialog=self._material_assembly_item_dialog,
            _open_profile_step_igs_quote_builder=self._open_profile_step_igs_quote_builder,
            _product_assembly_item_dialog=self._product_assembly_item_dialog,
            _quote_line_is_laser_2d=self._quote_line_is_laser_2d,
            _quote_pick_workcenter=self._quote_pick_workcenter,
            _structure_quote_dialog=self._structure_quote_dialog,
            _wrap_assembly_item=self._wrap_assembly_item,
        )
        return edit_calculated_assembly(self, ports, initial)
    ...
    def _assembly_model_editor_dialog(self, initial: dict | None = None) -> dict | None:
        ports = AssemblyModelEditorPorts(
            self._wrap_assembly_item, self._assembly_item_kind_from_line,
            self._assembly_item_kind_label, self._pick_assembly_item_kind,
            self._open_assembly_item_editor, self._quote_line_is_laser_2d,
            self._edit_laser_batch_lines,
        )
        return edit_assembly_model(self, ports, initial)

    def _manage_assembly_models(self) -> None:
        ports = AssemblyModelManagerPorts(
            self.services.assembly_model_rows, self.services.assembly_model_detail,
            self.services.assembly_model_save, self.services.assembly_model_remove,
            self._assembly_model_editor_dialog,
        )
        manage_assembly_models(self, ports)

    def _manage_saved_conjuntos(self) -> None:
        def append_lines(rows):
            self.line_rows.extend(rows)
            self._render_quote_lines()
        ports = SavedAssemblyPorts(
            self.services.conjunto_rows, self.services.conjunto_detail,
            self.services.conjunto_expand, self.services.conjunto_remove,
            self.services.conjunto_open_sheet_pdf,
            self._calculated_assembly_builder_dialog, append_lines,
        )
        manage_saved_assemblies(self, ports)

    def _save_selected_lines_as_group(self) -> None:
        selected = self._selected_line_indexes()
        ports = GroupEditorPorts(
            assembly_model_detail=self.services.assembly_model_detail,
            assembly_model_rows=self.services.assembly_model_rows,
            assembly_model_save=self.services.assembly_model_save,
            conjunto_detail=self.services.conjunto_detail,
            conjunto_rows=self.services.conjunto_rows,
            conjunto_save=self.services.conjunto_save,
            norm_text=self.services.norm_text,
            orc_line_is_piece=self.services.orc_line_is_piece,
            orc_line_is_product=self.services.orc_line_is_product,
            save_assembly_pair=self.services.save_assembly_pair,
            line_type_label=self._quote_line_type_label,
        )
        result = save_group(self, ports, self.line_rows, selected, self.current_number,
                            self.view.note_cliente_edit.text())
        if result is None:
            return
        self.line_rows = result.lines
        self._render_quote_lines()
        if selected:
            self._select_quote_line_source_index(selected[0])
        message = (
            f"As linhas selecionadas foram guardadas em {result.description} ({result.code})."
            if not result.overwrite
            else f"O registo {result.code} foi atualizado e as linhas ficaram ligadas ao conjunto/modelo."
        )
        QMessageBox.information(self, "Conjuntos", message)
